# Vibe Step 4b: Data Foundation

> God node · 14 connections · `tasks/vibe/step-4b-data-schema.md`

## Connections by Relation

### conceptually_related_to
- [[Circuit Breaker Pattern]] `INFERRED`
- [[Transactional Outbox Pattern]] `INFERRED`

### implements
- [[agentic-data-engineer Agent]] `EXTRACTED`

### references
- [[Zeitwerk Compliance]] `INFERRED`
- [[Vibe Step 1: Tree of Thoughts Exploration]] `EXTRACTED`
- [[Vibe Step 4: Project Scaffold]] `EXTRACTED`
- [[context-engineer Agent]] `INFERRED`
- [[Frozen String Literal Pragma]] `EXTRACTED`
- [[Systemic Functional Linguistics (SFL)]] `EXTRACTED`
- [[Embedding Dimension Locking]] `EXTRACTED`
- [[Content Hash Idempotency (SHA256)]] `EXTRACTED`
- [[HNSW Vector Index Strategy]] `INFERRED`
- [[GIN JSONB Index Strategy]] `INFERRED`

### shares_data_with
- [[Schema Step 2: Data Infrastructure Design]] `INFERRED`

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*